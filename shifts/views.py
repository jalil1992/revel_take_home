...
from .serializers import (ShiftAddVehicleSerializer, ShiftAutoCreateSerializer,
                          ShiftSerializer, ShiftStatusSerializer)
...
class ShiftAutoCreateView(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        vehicles = list(Vehicle.objects.exclude(in_use=True).filter(battery_level__lt=10))
        ...
        shift_vehicles = [ShiftVehicle(shift=shift, vehicle=ordered_vehicles[i], order=i) for i in range(len(ordered_vehicles))]
