...
class TestShift(RevelTestBase):
    ...
    def test_add_vehicles_manual(self):
        ...
        # add
        response = self.client.post(f"/shifts/{shift.id}/add-vehicle/", data)
        self.assertEquals(response.status_code, 200)
        self.assertTrue(response.json()["vehicles"] == vehicle_licenses)

        # check status
        response = self.client.get(f"/shifts/{shift.id}/check-status/")
        self.assertEquals(response.status_code, 200)
    ...
    def test_add_vehicles_auto(self):
        ...
        # create 100 vehicles, 20 lying on the circle of radius 20, 80 lying on the circle of radius 30
        start_point = (43.54, 34.52)

        vehicles = []
        for i in range(20):
            x = start_point[0] + 20 * math.cos(2 * math.pi * i / 20)
            y = start_point[1] + 20 * math.sin(2 * math.pi * i / 20)
            vehicles.append(
                Vehicle(license_plate=f"RANGE_20_{i}", battery_level=0, in_use=False, location_lat=x, location_long=y)
            )
        ...
        for i in range(80):
            ...
            vehicles.append(
                Vehicle(license_plate=f"RANGE_80_{i}", battery_level=0, in_use=False, location_lat=x, location_long=y)
            )
        ...
        response = self.client.post(f"/shifts/auto-create/", data=data)
        self.assertEquals(response.status_code, 200)
        print(response.json())
